...
The input data for this package consists of a dataset containing daily patient-level information during emergency department (ED) stay and hospitalization. The dataset should include variables such as unique patient and hospitalization identifiers, daily clinical variables (blood culture collection, use of vasopressor and invasive mechanical ventilation, antimicrobial treatments), and laboratory measurements. The data should be sorted within each patient chronologically by the admission date and hospital calendar day. The following table detailed the required variables:

| Variable Name | Description | Variable Calculation and Source |
|--------------|-------------|---------------------------------|
| `unique_pt_id` | Unique identifier for each patient.| Direct extraction; requires de-identification. |
| `seqnum` | Unique hospital visit identifier assigned to each admission for a patient. | Direct extraction; requires de-identification. |
| `day`   | Sequential hospital day number, where day 1 corresponds to the admission date. Values < 1 represent emergency department (ED) days prior to admission. | Direct extraction; requires de-identification. |
| `death` | Binary indicator: 1 = patient died on that day.   | Extracted from health records.  |
| `ALL_DAYS` | Number of calendar days during the hospitalization, excluding ED days.   | Calculated from admission and discharge dates. |
| `bcx_daily` | Binary indicator: 1 = a blood culture was collected on that day. | Extracted from laboratory data.|
| `vasop_daily` | Binary indicator: 1 = received a qualifying vasopressor by intravenous infusion for any duration during that calendar day. | Extracted from medication data. Refer to CDC ASE Toolkit (p. 15) for qualifying agents. |
| `imv_daily` | Binary indicator: 1 = had invasive mechanical ventilation for any duration during that calendar day. | Identified using procedure codes or ICU data. |
| `lact_daily_hi` | Highest serum lactate value measured on that calendar day (mmol/L). | Extracted from laboratory data. |
| `tbili_daily_hi` | Highest total bilirubin value measured on that calendar day (µmol/L). | Extracted from laboratory data. |
| `tbili_daily_lo` | Lowest total bilirubin value measured on that calendar day (µmol/L). | Extracted from laboratory data. |
| `tbili_baseline` | Lowest total bilirubin value measured during the entire hospital stay (µmol/L). | Extracted from laboratory data. |
| `creat_daily_hi` | Highest serum creatinine value measured on that calendar day (µmol/L). | Extracted from laboratory data. |
| `creat_daily_lo` | Lowest serum creatinine value measured on that calendar day (µmol/L). | Extracted from laboratory data. |
| `creat_baseline` | Lowest serum creatinine value measured during the entire hospital stay (µmol/L). | Extracted from laboratory data. |
| `plt_daily_hi` | Highest platelet count measured on that calendar day (10^9/L). | Extracted from laboratory data. |
| `plt_daily_lo` | Lowest platelet count measured on that calendar day (10^9/L). | Extracted from laboratory data. |
| `plt_baseline` | Highest platelet count measured during the entire hospital stay (10^9/L). | Extracted from laboratory data. |
| `esrd_icd` | Binary indicator: 1 = end-stage renal disease identified using ICD-10 codes. | Derived from ICD-10 codes (e.g., N18.5, N08.35, Z99.2 in Canada). |
| `new_abx_start` | Binary indicator: 1 = any new qualifying antimicrobial was first administered on that calendar day. | Derived from medication data. Refer to CDC ASE Toolkit (p. 14). |
| `abx_daily` | Binary indicator: 1 = received any qualifying antimicrobial on that calendar day. | Derived from medication data. Refer to CDC ASE Toolkit (p. 14). |
...
### `define_ase`
This is the main function users will interact with in the package. By invoking `define_ase`, users automatically trigger the sub-functions listed above and helper functions to sequentially evaluate the ASE toolkit criteria. This function contains parameters for lab measurement thresholds of creatinine, bilirubin, lactate, and platelet counts, as well as the minimum duration of qualifying antimicrobial treatment days. The function also provides an `ed_inclusion` parameter (integer 1–4) that controls the preprocessing of emergency department (ED) days in daily_data, including options to retain or exclude ED data. These parameters are preset to align with the ASE toolkit’s criteria but can be customized to align with specific research needs. Additionally, the function includes parameters such as `cohort_id` and `transferout_id`, which allow users to select subgroups and indicate patients who were transferred out to an acute hospital. 
...
| `transferout_id`   | A vector of sequence numbers (`seqnum`) indicating patients who were transferred out to an acute hospital. If no value, indicate transferout_id = NULL. |
...
| `creat_hi_cutoff`  | The cutoff value for high creatinine levels (default is NA, per the ASE toolkit; however, users may optionally specify a threshold value, such as 44.2 µmol/L from the Risk, Injury, Failure, Loss, and End-stage (RIFLE) renal disease classification system guidelines). |
...
| `ed_inclusion`  | An integer (1–4) specifying how to preprocess the ED rows in daily_data. 1 = ASE Toolkit default — include up to 2 ED rows (days) before hospital admission; 2 = Include all ED rows; 3 = Drop all ED rows; 4 = Drop ED rows, but if any ED blood culture was done, assign `bcx_daily = 1` on initial hospital day. |
...
data_path <- system.file("extdata", "example_input_data", 
                         "daily_data_mock.csv", package = "identifyASE")
...
# Example:
cohort_id <- list(c(2, 3), c(1222, 1333))
...
2. *"Data_(x)days"* includes data of ASE cases surrounding the selected (x) window (1-day,2-day, 3-day or 4-day). For example, if window = 2,  the output will include the dataset "Data_2days". If window = 3,  the output will include the dataset "Data_3days". The variables listed below are added to the dataset. The output dataset also contains additional derived variables that are used internally to support ASE case identification and onset classification, but are not exhaustively documented here. 
...
sliced_data_list <- list(
  list(unique_pt_id = 1, seqnum = 1111, data = data.frame(day = 1:3, 
                                                       bcx_daily = c(0, 1, 0))),
  list(unique_pt_id = 2, seqnum = 1222, data = data.frame(day = 1:3, 
                                                       bcx_daily = c(0, 1, 0)))
)
