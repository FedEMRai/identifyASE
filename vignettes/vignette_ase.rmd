...
| `vasop_daily` | Binary indicator: 1 = received a qualifying vasopressor on that day. | Extracted from medication data. Refer to CDC ASE Toolkit (p. 15) for qualifying agents. |
| `imv_daily` | Binary indicator: 1 = invasive mechanical ventilation on that day. If there was IMV on any ED day, imv_daily = 1 for all ED days. | Identified using procedure codes or ICU data. |
...
| `new_drug_cat` | Binary indicator: 1 = a qualifying antimicrobial was first administered on that day (not given in the prior 2 days). | Derived from medication data. Refer to CDC ASE Toolkit (p. 14). |
| `abx_daily` | Binary indicator: 1 = received any qualifying antimicrobial on that day. | Derived from medication data. Refer to CDC ASE Toolkit (p. 14). |
...
### `define_ase`
This is the main function users will interact with in the package. By invoking `define_ase`, users automatically trigger the sub-functions listed above and helper functions to sequentially evaluate the ASE toolkit criteria. This function contains parameters for lab measurement thresholds of creatinine, bilirubin, lactate, and platelet counts, as well as the minimum duration of qualifying antimicrobial treatment days. The function also provides an `ed_inclusion` parameter (integer 1–4) that controls preprocessing of emergency department (ED) days in daily_data, including options to retain or exclude ED data. These parameters are preset to align with the ASE toolkit’s criteria but can be customized to align with specific research needs. Additionally, the function includes parameters such as `cohort_id` and `transferout_id`, which allow users to select subgroups and indicate patients who were transferred out to an acute hospital. 
...
| `ed_inclusion`  | An integer (1–4) specifying how to preprocess the ED rows in daily_data. 1 = ASE Toolkit default — keep 2 days in ED; 2 = Include all ED rows; 3 = Drop all ED rows; 4 = Drop ED rows, but if any ED blood culture is detected, assign `bcx_daily = 1` on hospital day 1. |
